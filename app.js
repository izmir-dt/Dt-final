...
async function waitForFullData(fetcher, minRows = 700, maxTry = 10){
  for(let i=0;i<maxTry;i++){
    try{
      const rows = await fetcher();
      if(rows && rows.length >= minRows){
        console.log("DATA OK:", rows.length);
        return rows;
      }
      console.warn("Eksik veri geldi:", rows?.length, "yeniden deneniyor...");
    }catch(e){
      console.warn("Fetch hata, yeniden denenecek", e);
    }
    await new Promise(r=>setTimeout(r, 1200 + (i*400)));
  }
  throw new Error("Google Sheets sürekli eksik veri gönderdi");
}
...
// === SAFE COPY (clipboard + fallback) ===
async function idtCopyToClipboard(text){
