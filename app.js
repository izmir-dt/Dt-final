...
async function tryLoadApiFetch(){
  if(!CONFIG.API_BASE) throw new Error("API_BASE tanımlı değil");
  const url = `${CONFIG.API_BASE}?sheet=${encodeURIComponent(CONFIG.SHEET_MAIN)}&_=${Date.now()}`;
  const res = await fetch(url, {cache:"no-store", mode:"cors"});
  if(!res.ok) throw new Error(`API indirilemedi (${res.status}).`);
  ...
  })).filter(x=>x.play && (x.person || x.role || x.category));
}
...
async function load(isAuto=false){
  ...
  try{
    // 1) Apps Script API (CORS fetch)
    try{ rawRows = await tryLoadApiFetch(); }
    catch(e0){
      // 2) Apps Script API (JSONP callback)
    ...
    els.list.innerHTML = `
<div class="empty">
  <div class="big">Veri çekilemedi.</div>
  <div class="small">
    1) Sheet paylaşımı: Paylaş → <b>“Bağlantıya sahip herkes: Görüntüleyebilir”</b><br/>
    2) Apps Script Web App: Deploy → Web app → Execute as: Me, Who has access: Anyone<br/>
    3) Üstteki <b>API Test</b> butonundan JSON geliyor mu kontrol et<br/>
    4) Netlify / GitHub Pages’da genelde sorunsuz çalışır.
  </div>
  <div class="small" style="margin-top:10px;opacity:.9">Hata: ${escapeHtml(err.message || String(err))}</div>
</div>`;
    els.details.innerHTML = `<div class="empty"><div class="big">Önce veri gelsin 🙂</div></div>`;
    els.distributionBox.innerHTML = `<div class="empty"><div class="big">Veri yok.</div></div>`;
    els.figuranBox.innerHTML = `<div class="empty"><div class="big">Veri yok.</div></div>`;
    els.intersectionBox.innerHTML = `<div class="empty"><div class="big">Veri yok.</div></div>`;
  }
}
