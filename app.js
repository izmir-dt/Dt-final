...
let activePlayFilter = null;

let chartMode = "roles";
let chartHits = [];
let drawerData = [];
let drawerStack = [];
let drawerMode = "items";
...
function getDrawerFilteredItems(){
  ...
  if(labelMode) return drawerData.slice();
...
function processData(rawData) {
  ...
  try {
    const active = (window.__idtActiveTab || "") === "Assign" || 
                   (el("tabAssign") && el("tabAssign").classList.contains("active"));
    if(active) { 
      initAssignToolOnce(); 
      renderAssignTool(); 
    }
