...
function showManualCopy(text){
  ...
  if(!modal){
    ...
    modal.id = "copyModal";
    modal.innerHTML = `
      <div class="overlay" style="position:fixed;inset:0;background:rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;z-index:9999;">
        <div class="card" style="max-width:720px;width:min(92vw,720px);padding:14px 14px 12px;">
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;margin-bottom:8px">
            <div>
              <div class="small" style="line-height:1.25;margin-bottom:6px">İzmir Devlet Tiyatrosu Müdürlüğü<br>Repertuvar Kişi Takip Sistemi<br>Sanat Teknik Bürosu</div>
              <b>📋 Excel’e Kopyala</b>
              <div class="small">Ctrl+C / Kopyala → Excel’de Ctrl+V</div>
            </div>
    ...
    document.addEventListener("keydown", (e)=>{ if(e.key==="Escape" && document.getElementById("copyModal")) document.getElementById("copyModal").remove(); }, {once:true});
  }
...
async function copyText(text){
  ...
  setStatus("⚠️ Otomatik kopyalama engellendi. Açılan kutudan Ctrl+C yap.", "warn");
  return false;
}
...
function buildDetailsHtml(it){
  if(!it) return `<div class="empty">Soldan bir oyun veya kişi seç.</div>`;
  if(activeMode==="plays"){
    ...
    return `
      ...
      <div id="detailTable">
      ${blocks.map(([p, rs])=>{
        const rs2=[...rs].sort((a,b)=>
          (a.category||"").localeCompare(b.category||"","tr") ||
          (a.person||"").localeCompare(b.person||"","tr")
        );
        return `
          <div class="card" style="margin:10px 0">
            <div class="hd" style="padding:10px 12px"><b>${escapeHtml(p)}</b><span class="pill">${rs2.length} kişi</span></div>
            <div style="padding:0 12px 12px">
              <table class="table">
                <thead><tr><th>Kategori</th><th>Kişi</th></tr></thead>
                <tbody>
                  ${rs2.map(r=>`<tr><td>${escapeHtml(r.category)}</td><td>${escapeHtml(r.person)}${personTag(r.person)}</td></tr>`).join("")}
                </tbody>
              </table>
            </div>
          </div>`;
    ...
    const plays = [...new Set(rowsSorted.map(r=>r.play))];
    return `
      <h3 class="title">${escapeHtml(it.title)}${personTag(it.title)}</h3>
      <p class="subtitle">${plays.length} oyun • ${it.rows.length} satır</p>
      <table class="table" id="detailTable">
        <thead><tr><th>Oyun</th><th>Kategori</th><th>Görev</th></tr></thead>
        <tbody>
          ${rowsSorted.map(r=>`<tr><td>${escapeHtml(r.play)}</td><td>${escapeHtml(r.category)}</td><td>${escapeHtml(r.role)}</td></tr>`).join("")}
        </tbody>
      </table>
    `;
  }
...
function renderDetails(it){
  const html = buildDetailsHtml(it);
  if(isMobile()){
    // Mobilde: seçince detay altta modal olarak açılır → yukarı kaydırma derdi yok
    if(it) openMobileModal(html);
    else els.details.innerHTML = html;
    return;
  }
