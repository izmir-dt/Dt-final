...
/* --- Mobile: Liste yatay kaydırma (kart carousel) --- */
@media (max-width: 980px){
  #list{
    display:flex !important;
  ...
  /* Detay kartı üstte kalsın, yatay listede seçim rahat */
  .split{ grid-template-columns: 1fr !important; }
}


/* --- Inline play details accordion (mobile) --- */
.inlineDetails{ margin-top:10px; border:1px solid color-mix(in srgb, var(--line) 70%, transparent 30%); border-radius:14px; background: color-mix(in srgb, var(--card) 92%, var(--bg) 8%); overflow:hidden; }
.inlineDetails.hidden{ display:none; }
.inlineHd{ display:flex; align-items:center; justify-content:space-between; padding:10px 12px; border-bottom:1px solid color-mix(in srgb, var(--line) 70%, transparent 30%); }
.inlineTitle{ font-weight:800; }
.inlineBody{ padding:10px 12px; }
@media (max-width: 768px){
  .inlineBody table, .inlineBody thead, .inlineBody tbody, .inlineBody th, .inlineBody td, .inlineBody tr { display:block; }
  .inlineBody thead tr{ position:absolute; top:-9999px; left:-9999px; }
  .inlineBody tr{ border:1px solid var(--line); margin-bottom:10px; border-radius:12px; padding:10px; background: var(--card); }
  .inlineBody td{ border:none; position:relative; padding-left:50%; text-align:right; }
  .inlineBody td:before{ content: attr(data-label); position:absolute; left:10px; width:45%; text-align:left; font-weight:700; color:var(--muted); }
}
...
<header>
  <div class="top">
    <div class="brand">
      ...
      <div>
        <h1>İDT PANEL | <small style="font-weight:700;opacity:.85">SANAT TEKNİK BÜROSU</small></h1>
        </div>
    </div>
    <div class="actions">
...
<div class="wrap">
  <div class="kpis" id="kpis">
    ...
    <div class="kpi soft3" data-go="Panel" data-mode="rows" role="button" tabindex="0" aria-label="Panele git"><div><div class="label">Görev Ataması</div><div class="value" id="kpiRows">-</div></div><div class="icon">🧾</div></div>
