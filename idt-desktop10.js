...
(function(){
  ...
  document.addEventListener("click", async (e)=>{
    ...
    if(!tsv) return; // tablo yoksa mevcut handler'a bırak
  ...
  // Dropdownlar varsa otomatik Venn
  function wireVenn(){
    const a = document.getElementById("ccGameA");
    const b = document.getElementById("ccGameB");
    if(!a || !b) return;
    const run = ()=>{ if(a.value && b.value) window.IDT_V9_RENDER_VENN(a.value, b.value); };
